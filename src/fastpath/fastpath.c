...
void NORETURN fastpath_reply_recv_local(cap_t ep_cap, cap_t reply_cap, word_t length, seL4_MessageInfo_t info)
{
    ...
    if (unlikely(!cap_reply_cap_get_capCanTag(reply_cap))) {
    ...
    if (unlikely(fault_type != seL4_Fault_NullFault)) {
...
#endif /* CONFIG_EXCEPTION_FASTPATH */
        ...
        word_t msgInfo = wordFromMessageInfo(seL4_MessageInfo_set_capsUnwrapped(info, 0));
...
#endif
{
    ...
    if (unlikely(!coreCheckPreIPC(NODE_STATE(ksCurThread), ep_ptr))) {
        reply_object_lock_release(reply_ptr, "fastpath");
        /* Let the slowpath to raise fault for mismatched cores */
        slowpath(SysReplyRecv);
    }
#endif /* ENABLE_SMP_SUPPORT */
#endif /* CONFIG_CORE_TAGGED_OBJECT */
    ...
    /*
     * Should we remove it?
     * For active receivers, as they are not allowed to receive on untagged endpoint,
     * it's reasonable for the senders to assume that most of the destination threads
     * are passive. However, the receivers don't know whether the senders are active
     * or not, so maybe we should keep this branch here.
     */
#ifdef CONFIG_KERNEL_MCS
    if (unlikely(caller->tcbSchedContext != NULL)) {
        ep_lock_release(ep_ptr);
...
#ifdef CONFIG_FINE_GRAINED_LOCKING
        slowpath_exclusive(SysReplyRecv);
#else
        slowpath(SysReplyRecv);
...
#ifdef CONFIG_CORE_TAGGED_OBJECT
    if (cap_endpoint_cap_get_capCanTag(ep_cap)) {
        fastpath_reply_recv_local(ep_cap, reply_cap, length, info);
    } else {
#endif /* CONFIG_CORE_TAGGED_OBJECT */
#ifdef CONFIG_KERNEL_MCS
        fastpath_reply_recv_normal(ep_cap, reply_cap, length, info);
#else
        fastpath_reply_recv_normal(ep_cap, length, info);
#endif /* CONFIG_KERNEL_MCS */
#ifdef CONFIG_CORE_TAGGED_OBJECT
    }
#endif /* CONFIG_CORE_TAGGED_OBJECT */
